...
import MenuComponent from '@/components/FooterComponent.vue'
...
//alert(JSON.stringify(router.currentRoute.value.meta));
	...
	if ((event.target as HTMLElement).id) {
		...
		triggeredSlideIndex.value = slideItem.value.findIndex((slideitem) => {
			switch (id) {
				...
				case 'material-item':
					return Object.keys(watchStore.materialDict).find(
						(key) =>
							watchStore.materialDict[key] === (event.target as HTMLElement).innerText
					)
				case 'brand-item':
					return false
				default:
					console.error(
						'Выбранный элемент меню не соответствует ни одному из условий. Проверьте handleSubItem в StartPage. Event:' +
							event
					)
					break
			}
...
const changeToSize = async () => {
	...
	setTimeout(() => {
		...
		slideItem.value = watchStore.newCase
			.filter((item) => item.series === watchStore.currentWatch.series)
...
const changeToCase = async () => {
	...
	setTimeout(() => {
		watchStore.state = State.CASE_CHOICE
		slideItem.value = watchStore.newCase.filter(
			(item) =>
				item.series === watchStore.currentWatch.series &&
				item.size === watchStore.currentWatch.caseSize
		)
		document.querySelector('.animated-fade')!.classList.remove('fade-out')
	}, 1000)
}
...
const changeToBand = async () => {
	...
	setTimeout(() => {
		...
		slideItem.value = watchStore.newBands.filter((item) => {
			return parseInt(watchStore.currentWatch.caseSize!) <= 41
				? [38, 40, 41].includes(parseInt(item.size))
				: [42, 44, 45, 49].includes(parseInt(item.size))
		})
